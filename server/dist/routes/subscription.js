...
async function subscriptionRoutes(fastify) {
    fastify.post("/checkout", { preHandler: [fastify.authenticate] }, async (request, reply) => {
        try {
            ...
            if (!planId || !stripe_plans_1.SUBSCRIPTION_PLANS[planId]) {
            ...
            if (user.stripeCustomerId && user.stripeSubscriptionId) {
                try {
                    const subscription = await stripe_1.stripe.subscriptions.retrieve(user.stripeSubscriptionId);
            ...
            const plan = stripe_plans_1.SUBSCRIPTION_PLANS[planId];
    ...
    fastify.post("/cancel", { preHandler: [fastify.authenticate] }, async (request, reply) => {
        try {
            ...
            fastify.log.info(`Assinatura agendada para cancelamento: usuário ${userId}`);
            return reply.send({
                success: true,
                message: "Assinatura cancelada. Você manterá os benefícios até o fim do período.",
            });
        }
        catch (error) {
    ...
    fastify.post("/webhook", {
        ...
        try {
            ...
            let event;
            try {
                event = stripe_1.stripe.webhooks.constructEvent(request.rawBody, sig, process.env.STRIPE_WEBHOOK_SECRET);
            }
            catch (err) {
            ...
            switch (event.type) {
                case "checkout.session.completed": {
                    ...
                    if (session.metadata?.userId) {
                        ...
                        fastify.log.info(`Customer ID salvo para usuário ${session.metadata.userId}`);
                    }
                    break;
                }

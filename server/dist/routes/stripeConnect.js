...
async function stripeConnectRoutes(fastify) {
        ...
        try {
            ...
            if (user.stripeAccountId) {
                ...
                return reply.send({
                    ...
                    accountStatus: account.capabilities?.transfers === "active"
                        ? "active"
                        : "pending",
                    accountLink: null,
                });
    ...
    fastify.get("/debug-balance", {
        ...
        try {
            ...
            const balanceTransactions = await stripe_1.stripe.balanceTransactions.list({ limit: 10 }, { stripeAccount: user.stripeAccountId });
            // Buscar últimos payouts
            const payouts = await stripe_1.stripe.payouts.list({ limit: 10 }, { stripeAccount: user.stripeAccountId });
            return reply.send({
                userId,
                stripeAccountId: user.stripeAccountId,
    ...
    fastify.post("/webhook", {
        ...
        let event;
        try {
            event = stripe_1.stripe.webhooks.constructEvent(request.rawBody, sig, process.env.STRIPE_WEBHOOK_SECRET_CONNECT);
        }
        catch (err) {
            ...
            switch (event.type) {
                case "account.updated":
                    ...
                    await fastify.prisma.user.updateMany({
                        ...
                        data: {
                            stripeAccountStatus: account.capabilities?.transfers === "active"
                                ? "active"
                                : "pending",
                        },
                    });
                    fastify.log.info(`Account ${account.id} updated to status: ${account.capabilities?.transfers === "active"
                        ? "active"
                        : "pending"}`);
                    break;
                case "payout.paid":
                ...
                case "payout.failed":
                    ...
                    const failedTransaction = await fastify.prisma.transaction.findFirst({
                        where: {
                            metadata: {
                                path: ["stripePayoutId"],
                    ...
                    fastify.log.error(`Payout ${failedPayout.id} failed: ${failedPayout.failure_message}`);
                    break;
                default:

...
async function tournamentRoutes(fastify) {
    fastify.post("/", {
        ...
        try {
            ...
            if (!(0, roleHelper_1.isPremiumOrAdmin)(user.role)) {
    ...
    fastify.get("/", async (request, reply) => {
        try {
            ...
            const tournamentsWithPrizePool = tournaments.map((t) => {
                const paidParticipants = t.participants.filter((p) => p.paidEntry).length;
                return {
                    ...t,
    ...
    fastify.get("/:id", async (request, reply) => {
        try {
            ...
            const paidParticipants = tournament.participants.filter((p) => p.paidEntry).length;
    ...
    fastify.post("/:id/join", {
        ...
        try {
            ...
            const existingParticipant = await fastify.prisma.tournamentParticipant.findUnique({
                where: {
                    tournamentId_userId: {
                        tournamentId,
                        userId,
            ...
            if (user.role === "ADMIN") {
                return reply.status(403).send({
                    ...
                    message: "Admins podem apenas criar e gerenciar torneios, mas não participar como jogadores.",
                });
            }
    ...
    fastify.delete("/:id/leave", {
        ...
        try {
            ...
            if (tournament.creatorId === userId) {
                return reply.status(400).send({
                    error: "O criador do torneio não pode sair. Considere cancelar o torneio.",
                });
            }
    ...
    fastify.post("/:tournamentId/match/:matchId/start", {
        preHandler: [fastify.authenticate],
    }, async (request, reply) => {
        try {
            ...
            const orchestrator = new tournamentOrchestrator_1.TournamentOrchestrator(fastify.prisma, fastify.wsManager, fastify.log);

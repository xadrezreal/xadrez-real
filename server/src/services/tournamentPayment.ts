 ...
 * - etc.
 */
function calculateFinalStandings(tournament: any): { userId: string; finalPosition: number }[] {
  ...
  const standings: { userId: string; finalPosition: number }[] = [];
 ...
 */
export async function enterTournamentWithConnect(
  userId: string,
  tournamentId: string,
  entryFee: number
) {
  ...
  if (user.balance < entryFee) {
    throw new Error(
      `Saldo insuficiente. Necessário: R$ ${entryFee.toFixed(2)}. ` +
